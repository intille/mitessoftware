...
namespace OpenNETCF.IO.Serial
{
	...
	public class Port : IDisposable
	{
		...
		private int rxBufferSize = MAX_BUFFER_SIZE;
		private Queue rxFIFO;
		private int rthreshold = 1;

		// default Tx buffer is 1024 bytes
		private int txBufferSize = MAX_BUFFER_SIZE;
		...
		/// <summary>
		/// Open the current port using NETCF standard method. Defaults to threaded. 
		/// </summary>
		/// <returns>true if successful, false if it fails</returns>
		public bool Open()
		{
			return OpenThreaded(true);
		}
...
*/
		...
		private void CommEventThread()
		{
			...
			try
			{
				...
				while(hPort != (IntPtr)CommAPI.INVALID_HANDLE_VALUE)
				{
					...
					// SSI Hack -- need to fix so only active on phone
					eventFlags = CommEventFlags.RXCHAR;
					...
					{
						tmp = m_CommAPI.SetCommMask(hPort, CommEventFlags.ALLCE);
					}
		...
		private int bread = 0;
		private byte[] grabBytes = new byte[MAX_BUFFER_SIZE]; 
		
		CommEventFlags	grabEventFlags	= new CommEventFlags();
		bool isNotPending = true;
