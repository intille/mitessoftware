...
namespace HousenCS.MITes
{
	...
	public class MITesReceiverController
	{
        ...
        private const bool PRINT_WARNINGS = true; // For TestPort
	
		private const int MAX_PORT_NUM = 18;
		...
		/// some point. 
		/// </summary>
		public const int MAXBYTESBUFFER = 1800; 
		...
		private const char messageRxChannel = '9';  //message sent to set the receiver channel
		private const char messageRxGetChannels = '0'; // message sent to get the receiver channels
        ...
        public MITesReceiverController(int aPortNumber, int maxBytesBuffer, bool printWarnings, bool isUsingThreads)
        {
            portNumber = aPortNumber;

            serialBytesBuffer = new byte[maxBytesBuffer];

            if (portNumber == FIND_PORT)
                for (int i = 1; i <= MAX_PORT_NUM; i++)
                    if (portNumber == FIND_PORT)
                        if (TestPort(i, maxBytesBuffer))
                        {
                            portNumber = i;
                            //call the progress bar delegate
                        }

            if (portNumber == FIND_PORT)
            {
                Console.WriteLine("Could not find a valid COM port with a MITes receiver!");
                //Application.Exit();
                isRunning = false;
                spc = null;
            }
        ...
        public delegate void UpdateProgressDelegate(string description, int step);
        static int receiverCount=0;

        /// <summary>
        /// 
        /// </summary>
        /// <param name="aPortNumber"></param>
        /// <param name="maxBytesBuffer"></param>
        /// <param name="printWarnings"></param>
        /// <param name="isUsingThreads"></param>
        /// <param name="progressCallback"></param>
		public MITesReceiverController(int aPortNumber, int maxBytesBuffer, bool printWarnings, bool isUsingThreads,UpdateProgressDelegate progressCallback)
		{
			...
			if (portNumber == FIND_PORT)
				for (int i = 1; i <= MAX_PORT_NUM; i++)
                    if (portNumber == FIND_PORT)
                    {                        
                        //Call delegate to update progress bar
                        progressCallback("Receiver "+ receiverCount+": Searching COM port " + i + " ...", 1 );
                        if (TestPort(i, maxBytesBuffer))                        
                            portNumber = i;
			...
			{
				Console.WriteLine("Receiver found on COM " + portNumber + "!");                
                progressCallback("Receiver found on COM " + portNumber +" ...", MAX_PORT_NUM-portNumber);
				// Checked that port is valid, so go ahead and reopen it. 
				spc = new SerialPortController(isUsingThreads, printWarnings,maxBytesBuffer);
				Thread.Sleep(1000);
				spc.SetBaudRate(57600);
		...
		/// <returns></returns>
		private bool TestPort(int portNum, int maxBytesBuffer)
		{
			isRunning = true; 
