...
namespace HousenCS.MITes
{
	...
	public class MITesScalablePlotter
	{
		...
		/// 
		/// </summary>
		public static int MAXRETURNVALS = 40 * 7; // 10 works, 4 is NUMSENSORS
		...
		public MITesScalablePlotter(System.Windows.Forms.Form aForm, DeviceTypes aDeviceType, int maxPlots, MITesDecoder aMITesDecoder, Size aSize)
		{
			...
			for (int j = 0; j < numVals; j = j+4)
			{
				if (returnVals[j] != MITesDecoder.STATIC_CHANNEL)
				{
					...
					if (id < maxPlots)
					{
						...
						{
							aForm.Invalidate(new System.Drawing.Rectangle(col-1,0,2,plotAreaSize.Height));
							col++;
		...
		/// </summary>
        /// <param name="g"></param>
        /// <param name="isUseLines">True if draw with lines, false if use points</param>
        public void DrawValsFast(Graphics g, bool isUseLines)
		{			
				...
				for (int c = lastColDrawn; c < plotAreaSize.Width; c++)
				{
					for (int i = 0; i < maxPlots;i++) //SSI maxPlots correct?
						for (int j = 0; j < 3; j++)
						{
                            ...
                            {
                                if (v1 != MITesData.EMPTY)
                                {
                                    g.DrawLine(p[j], c + 4, v1, c+4, v1+1); // 4 is gapDistance
                                    //g.DrawRectangle(p[i*3 + j],dc,v1,1,1);
                                }
                            }                                
						}
