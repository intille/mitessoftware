...
namespace HousenCS.MITes
{
	...
	public class MITesDataFilterer
	{
        ...
        /// <summary>
        /// Store tracking info for each accelerometer channel and HR
        /// </summary>
        public MITesPerformanceStats[] MitesPerformanceTracker = new MITesPerformanceStats[MITesData.MAX_MITES_CHANNELS];

				private int nonNoiseCount = 0;
        private int nonNoiseHR = 0;
        private int nonNoiseACCEL = 0;

        /// <summary>
        /// 
        /// </summary>
        public int NonNoiseHR
        {
            get
            {
                return this.nonNoiseHR;
            }
        }
        /// <summary>
        /// 
        /// </summary>
        public int NonNoiseACCEL
        {
            get
            {
                return this.nonNoiseACCEL;
            }
        }
            ...
            for (int i = 0; i < aMITesDecoder.someMITesDataIndex; i++)
            {
                if (aMITesDecoder.someMITesData[i].type != (int)MITesTypes.NOISE)
                {
                    if (aMITesDecoder.someMITesData[i].type==(int) MITesTypes.ACCEL){                        
                        this.MitesPerformanceTracker[(int)aMITesDecoder.someMITesData[i].channel].SampleCounter++;                               
                        this.nonNoiseACCEL++;
                    }
